...
def _transform_image_batches(value, params, kwargs, context, model_def, config):
    return _image_batches(value)


def _transform_count(value, params, kwargs, context, model_def, config):
    return len(value or [])


def _transform_collect_counted_inputs(
    value, params, kwargs, context, model_def, config
):
    ...
    for index in range(2, input_count + 1):
        ...
        collected.extend(
            _collect_transformed_items(
                extra_value, item_transform, kwargs, context, model_def, config
            )
        )
    return collected


def _transform_upload_media_list(value, params, kwargs, context, model_def, config):
...
def _transform_resolve_size(value, params, kwargs, context, model_def, config):
    size = str(value)
...
def _transform_parse_bbox_list(value, params, kwargs, context, model_def, config):
    image_count = int(
...
def _transform_parse_color_palette(value, params, kwargs, context, model_def, config):
...
def _apply_transform(
    ...
    return handler(value, params, kwargs, context, model_def, config)
