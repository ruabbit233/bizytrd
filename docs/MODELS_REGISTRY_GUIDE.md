...
- `inputcountParam`
  - 正式字段，不是兼容字段
  - 当媒体参数需要读取一个现有 `INT` 参数来决定实际启用多少个输入口时使用
  - 如果未填写，并且参数名命中自动规则，系统会自动补出 `<media>_inputcount`
  - `inputcountParam` 指向的 `INT` 参数应通过它自己的 `max` 字段定义上限
...
- `sendIf`
  ...
  - 当前支持：
    ...
    - `nonzero`：值不等于 `0` 才发送
    - `not_default`：值与 `defaultValue` 不同才发送
    - `always`：总是发送
- `skipValues`
  - 一个数组
